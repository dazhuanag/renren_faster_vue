    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/ecourse/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.majorId = data.eCourse.majorId
                this.dataForm.name = data.eCourse.name
                this.dataForm.deleted = data.eCourse.deleted
                this.dataForm.createTime = data.eCourse.createTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/ecourse/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
