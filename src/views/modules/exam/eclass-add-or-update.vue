    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/eclass/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.majorId = data.eClass.majorId
                this.dataForm.name = data.eClass.name
                this.dataForm.deleted = data.eClass.deleted
                this.dataForm.createTime = data.eClass.createTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/eclass/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
